...
VALID_MFECLASSES = (
    ...
    autocorr.MFETSAutocorr,
    randomize.MFETSRandomize,
    info_theory.MFETSInfoTheory,
)  # type: t.Tuple
...
def process_generic_set(
    ...
    if values is None:
        ...
        raise ValueError(
            '"Values" can not be None. (while checking '
    ...
    if values is not None and not values:
        ...
        raise ValueError(
            '"Values" can not be empty. (while checking '
            'group "{}")'.format(group_name)
        )
...
def check_data(ts: t.Union[np.ndarray, list]) -> np.ndarray:
    """Checks ``ts`` data type and shape and transform it if necessary.

    Args:
        Check ``mfe.fit`` method for more information.

    Raises:
        TypeError: if ``ts`` is neither a np.ndarray nor a list-type object.

        ValueError: if ``ts`` is empty.

    Returns:
        np.ndarray: ``ts`` possibly reshaped and casted to
        :obj:`np.ndarray` type.
    """
    if not isinstance(ts, (np.ndarray, list)):
...
def transform_num(
    ...
    if num_bins is not None:
        ...
        if num_bins <= 0:
            raise ValueError(
                '"num_bins" must be a positive' "integer or NoneType."
            )
...
def check_score(score: str, groups: t.Tuple[str, ...]):
    ...
    """
    valid_scoring = scoring.VALID_SCORING
